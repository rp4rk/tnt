...
const submitEntries = (entries) => {
  ...
  entries.forEach((entry, i) => {
    setTimeout(() => fetch(getTimeEntryEndpoint(host), {
      method: 'POST',
      headers: {
...
const SummaryModal = ({ entries, comments, activities, visible, setShowModal }) => {
  const [summary, components] = getSummary(entries, comments, activities);

  return (
    <Modal title="Entries Summary" okText='Submit' visible={visible} onOk={() => submitEntries(summary)} onCancel={() => setShowModal(false)}>
      {components}
    </Modal>
  );
};
